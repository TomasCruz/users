...
func (h HTTPHandler) validateCreateUser(req CreateUserReq) error {
	...
	if countryLength != 3 && countryLength != 2 {
		...
		h.logger.Error(err, fmt.Sprintf("wrong country length %d", countryLength))
		return err
	}
...
// 	if countryLength != 3 && countryLength != 2 {
// 		err := errors.WithStack(entities.ErrCountryLength)
// 		logger.Error(err, fmt.Sprintf("wrong country length %d", strconv.Itoa(countryLength)))
// 		return err
// 	}
